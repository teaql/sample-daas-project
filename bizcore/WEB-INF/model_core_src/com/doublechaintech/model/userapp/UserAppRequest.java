...
import com.doublechaintech.model.listaccess.ListAccessRequest;
import com.doublechaintech.model.quicklink.QuickLinkRequest;
import com.doublechaintech.model.secuser.SecUserRequest;



import java.util.stream.Collectors;
import static com.doublechaintech.model.userapp.UserApp.*;

public class UserAppRequest extends BaseRequest<UserApp> {
    public static UserAppRequest newInstance() {
        return new UserAppRequest().filterByVersion(QueryOperator.GREATER_THAN,0).selectId();
    }
    ...
    public UserAppRequest addDynamicProperty(String propertyName, BaseRequest request, String parentPropertyName) {
    ...
    public UserAppRequest selectAny() {return selectAll().selectQuickLinkList(Q.quickLink().selectSelf()).selectListAccessList(Q.listAccess().selectSelf());
        }
    ...
    protected static final String[] ALL_SELF_FIELDS={ID_PROPERTY,TITLE_PROPERTY,SEC_USER_PROPERTY,APP_ICON_PROPERTY,FULL_ACCESS_PROPERTY,PERMISSION_PROPERTY,APP_TYPE_PROPERTY,APP_ID_PROPERTY,CTX_TYPE_PROPERTY,CTX_ID_PROPERTY,LOCATION_PROPERTY,VERSION_PROPERTY};
   ...
   public UserAppRequest count(){
    ...
    public UserAppRequest hasQuickLink(QuickLink... quickLink) {
       if (isEmptyParam(quickLink)){
        ...
        return hasQuickLink(Q.quickLink().select(QuickLink.APP_PROPERTY).filterById(QueryOperator.IN, (Object[])quickLink));
    }
    ...
    public UserAppRequest hasQuickLink(QuickLinkRequest quickLink) {
        return hasQuickLink(quickLink, $quickLink -> java.util.Optional.of($quickLink).map(QuickLink::getApp).map(UserApp::getId).orElse(null));
    }
    ...
    public UserAppRequest hasQuickLink(
      ...
      return addSearchCriteria(
          new AggregationCriteria(
              quickLink, QuickLink.APP_PROPERTY, queryOperator, value));
    }
    ...
    public UserAppRequest hasListAccess(ListAccess... listAccess) {
       if (isEmptyParam(listAccess)){
        ...
        return hasListAccess(Q.listAccess().select(ListAccess.APP_PROPERTY).filterById(QueryOperator.IN, (Object[])listAccess));
    }
    ...
    public UserAppRequest hasListAccess(ListAccessRequest listAccess) {
        return hasListAccess(listAccess, $listAccess -> java.util.Optional.of($listAccess).map(ListAccess::getApp).map(UserApp::getId).orElse(null));
    }
    ...
    public UserAppRequest hasListAccess(
      ...
      return addSearchCriteria(
          new AggregationCriteria(
              listAccess, ListAccess.APP_PROPERTY, queryOperator, value));
    }
