...
import com.doublechaintech.model.publickeytype.PublicKeyTypeRequest;
import com.doublechaintech.model.secuser.SecUserRequest;
import com.doublechaintech.model.userallowlist.UserAllowListRequest;



import java.util.stream.Collectors;
import static com.doublechaintech.model.userdomain.UserDomain.*;

public class UserDomainRequest extends BaseRequest<UserDomain> {
    public static UserDomainRequest newInstance() {
        return new UserDomainRequest().filterByVersion(QueryOperator.GREATER_THAN,0).selectId();
    }
    ...
    public UserDomainRequest addDynamicProperty(String propertyName, BaseRequest request, String parentPropertyName) {
    ...
    public UserDomainRequest selectAny() {return selectAll().selectUserAllowListList(Q.userAllowList().selectSelf()).selectSecUserList(Q.secUser().selectSelf()).selectPublicKeyTypeList(Q.publicKeyType().selectSelf());
        }
   ...
   public UserDomainRequest count(){
    ...
    public UserDomainRequest hasUserAllowList(UserAllowList... userAllowList) {
       if (isEmptyParam(userAllowList)){
        ...
        return hasUserAllowList(Q.userAllowList().select(UserAllowList.DOMAIN_PROPERTY).filterById(QueryOperator.IN, (Object[])userAllowList));
    }

    public UserDomainRequest hasUserAllowList(String... userAllowList) {
        return hasUserAllowList(Q.userAllowList().select(UserAllowList.DOMAIN_PROPERTY).filterById(userAllowList));
    }

    public UserDomainRequest hasUserAllowList(UserAllowListRequest userAllowList) {
        return hasUserAllowList(userAllowList, $userAllowList -> java.util.Optional.of($userAllowList).map(UserAllowList::getDomain).map(UserDomain::getId).orElse(null));
    }
    ...
    public SearchCriteria createUserAllowListCriteria(UserAllowListRequest userAllowList, IDRefine<UserAllowList> idRefine){
       return new RefinedIdInCriteria(userAllowList, ID_PROPERTY, idRefine, UserAllowList.DOMAIN_PROPERTY);
    }
    ...
    public UserDomainRequest hasSecUser(SecUser... secUser) {
       if (isEmptyParam(secUser)){
        ...
        return hasSecUser(Q.secUser().select(SecUser.DOMAIN_PROPERTY).filterById(QueryOperator.IN, (Object[])secUser));
    }
    ...
    public UserDomainRequest hasSecUser(SecUserRequest secUser) {
        return hasSecUser(secUser, $secUser -> java.util.Optional.of($secUser).map(SecUser::getDomain).map(UserDomain::getId).orElse(null));
    }
    ...
    public UserDomainRequest hasSecUser(
      ...
      return addSearchCriteria(
          new AggregationCriteria(
              secUser, SecUser.DOMAIN_PROPERTY, queryOperator, value));
    }
    ...
    public UserDomainRequest hasPublicKeyType(PublicKeyType... publicKeyType) {
       if (isEmptyParam(publicKeyType)){
        ...
        return hasPublicKeyType(Q.publicKeyType().select(PublicKeyType.DOMAIN_PROPERTY).filterById(QueryOperator.IN, (Object[])publicKeyType));
    }

    public UserDomainRequest hasPublicKeyType(String... publicKeyType) {
        return hasPublicKeyType(Q.publicKeyType().select(PublicKeyType.DOMAIN_PROPERTY).filterById(publicKeyType));
    }

    public UserDomainRequest hasPublicKeyType(PublicKeyTypeRequest publicKeyType) {
        return hasPublicKeyType(publicKeyType, $publicKeyType -> java.util.Optional.of($publicKeyType).map(PublicKeyType::getDomain).map(UserDomain::getId).orElse(null));
    }
    ...
    public SearchCriteria createPublicKeyTypeCriteria(PublicKeyTypeRequest publicKeyType, IDRefine<PublicKeyType> idRefine){
       return new RefinedIdInCriteria(publicKeyType, ID_PROPERTY, idRefine, PublicKeyType.DOMAIN_PROPERTY);
    }
  ...
  public UserDomainRequest statsFromUserAllowList(String retName, UserAllowListRequest userAllowList){
  ...
  public UserDomainRequest statsFromPublicKeyType(String retName, PublicKeyTypeRequest publicKeyType){
