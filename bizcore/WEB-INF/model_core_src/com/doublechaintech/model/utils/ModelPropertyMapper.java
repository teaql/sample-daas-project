...
public class ModelPropertyMapper {
    protected static Map<String, ModelBeanCreator> customCreators = new HashMap<>();
    protected ModelUserContext userContext;

    public ModelUserContext getUserContext() {
      return userContext;
    }
    public void setUserContext(ModelUserContext userContext) {
      this.userContext = userContext;
    }

    public static void registerCreator(Class beanClass, ModelBeanCreator creator) {
      registerCreator(beanClass, null, creator);
    }
    public static void registerCreator(Class beanClass, CreationScene scene, ModelBeanCreator creator) {
      registerCreator(beanClass, null, null, creator);
    }
    public static void registerCreator(Class beanClass, CreationScene scene, String propertyName, ModelBeanCreator creator) {
    ...
    public <T> ModelBeanCreator<T> findCustomCreator(Class<T> beanClass) {
      return findCustomCreator(beanClass, null);
    }

    public <T> ModelBeanCreator<T> findCustomCreator(Class<T> beanClass, CreationScene scene) {
      return findCustomCreator(beanClass, scene, null);
    }

    public <T> ModelBeanCreator<T> findCustomCreator(Class<T> beanClass, CreationScene scene, String propertyName) {
      String key = calcCreatorKey(beanClass, scene, propertyName);
      ModelBeanCreator<T> creator = customCreators.get(key);
    ...
    protected <T,B> T tryToGetFrom(Class beanClass, String propertyName, Class<T> propertyType, int indexInType, boolean onlyOneInType, T defaultValue, List<Object> inputs) {
        ...
        ModelBeanCreator<B> customCreator = findCustomCreator(beanClass, scene, propertyName);
