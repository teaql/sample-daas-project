...
public abstract class BaseRequest<T extends BaseEntity> implements Cloneable {
  ...
  public void selectParent(String name, BaseRequest request) {
    if (!name.equals(ModelBaseUtils.getParentLinkName(getInternalType()))) {
      ModelBaseUtils.enhanceRequest(request);
    }
    request.unlimited();
    select(name);
  ...
  public void selectChild(String name, BaseRequest request) {
    if (!name.equals(ModelBaseUtils.getParentLinkName(request.getInternalType()))) {
      ModelBaseUtils.enhanceRequest(request);
    }
