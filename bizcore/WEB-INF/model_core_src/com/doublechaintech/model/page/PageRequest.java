...
import com.doublechaintech.model.mobileapp.MobileAppRequest;
import com.doublechaintech.model.pagetype.PageTypeRequest;
import com.doublechaintech.model.section.SectionRequest;
import com.doublechaintech.model.slide.SlideRequest;
import com.doublechaintech.model.uiaction.UiActionRequest;



import java.util.stream.Collectors;
import static com.doublechaintech.model.page.Page.*;

public class PageRequest extends BaseRequest<Page> {
    public static PageRequest newInstance() {
        return new PageRequest().filterByVersion(QueryOperator.GREATER_THAN,0).selectId();
    }
    ...
    public PageRequest addDynamicProperty(String propertyName, BaseRequest request, String parentPropertyName) {
    ...
    public PageRequest selectAny() {return selectAll().selectSlideList(Q.slide().selectSelf()).selectUiActionList(Q.uiAction().selectSelf()).selectSectionList(Q.section().selectSelf());
        }
    ...
    protected static final String[] ALL_SELF_FIELDS={ID_PROPERTY,PAGE_TITLE_PROPERTY,LINK_TO_URL_PROPERTY,PAGE_TYPE_PROPERTY,DISPLAY_ORDER_PROPERTY,MOBILE_APP_PROPERTY,VERSION_PROPERTY};
   ...
   public PageRequest count(){
    ...
    public PageRequest whereDisplayOrderIsNotNull() {
      ...
      public PageRequest whereDisplayOrderBetween(Integer displayOrderStart, Integer displayOrderEnd) {
          if (ObjectUtil.isEmpty(displayOrderStart)) {
               throw new IllegalArgumentException("Method whereDisplayOrderBetween, the parameter displayOrderStart is not allowed  to be null.");
          }
          if (ObjectUtil.isEmpty(displayOrderEnd)) {
               throw new IllegalArgumentException("Method whereDisplayOrderBetween, the parameter displayOrderEnd is not allowed  to be null.");
          }
          SearchCriteria searchCriteria = getDisplayOrderSearchCriteria(QueryOperator.BETWEEN,  new Object[]{displayOrderStart, displayOrderEnd});
          return addSearchCriteria(searchCriteria);
      }
      ...
      public PageRequest whereDisplayOrderLessThan(Integer displayOrder) {
          if (ObjectUtil.isEmpty(displayOrder)) {
              throw new IllegalArgumentException("Method whereDisplayOrderLessThan, the parameter displayOrder is not allowed  to be null.");
          }
          SearchCriteria searchCriteria = getDisplayOrderSearchCriteria(QueryOperator.LESS_THAN, new Object[]{displayOrder});
          return addSearchCriteria(searchCriteria);
      }
      ...
      public PageRequest whereDisplayOrderGreaterThan(int displayOrder) {
          if (ObjectUtil.isEmpty(displayOrder)) {
              throw new IllegalArgumentException("Method whereDisplayOrderGreaterThan, the parameter displayOrder is not allowed  to be null.");
          }
          SearchCriteria searchCriteria = getDisplayOrderSearchCriteria(QueryOperator.GREATER_THAN, new Object[]{displayOrder});
          return addSearchCriteria(searchCriteria);
      }
    ...
    public PageRequest hasSlide(Slide... slide) {
       if (isEmptyParam(slide)){
        ...
        return hasSlide(Q.slide().select(Slide.PAGE_PROPERTY).filterById(QueryOperator.IN, (Object[])slide));
    }
    ...
    public PageRequest hasSlide(SlideRequest slide) {
        return hasSlide(slide, $slide -> java.util.Optional.of($slide).map(Slide::getPage).map(Page::getId).orElse(null));
    }
    ...
    public PageRequest hasSlide(
        SlideRequest slide, QueryOperator queryOperator, Object value) {
      return addSearchCriteria(
          new AggregationCriteria(
              slide, Slide.PAGE_PROPERTY, queryOperator, value));
    }
    ...
    public PageRequest hasUiAction(UiAction... uiAction) {
       if (isEmptyParam(uiAction)){
        ...
        return hasUiAction(Q.uiAction().select(UiAction.PAGE_PROPERTY).filterById(QueryOperator.IN, (Object[])uiAction));
    }
    ...
    public PageRequest hasUiAction(UiActionRequest uiAction) {
        return hasUiAction(uiAction, $uiAction -> java.util.Optional.of($uiAction).map(UiAction::getPage).map(Page::getId).orElse(null));
    }
    ...
    public PageRequest hasUiAction(
      ...
      return addSearchCriteria(
          new AggregationCriteria(
              uiAction, UiAction.PAGE_PROPERTY, queryOperator, value));
    }
    ...
    public PageRequest hasSection(Section... section) {
       if (isEmptyParam(section)){
        ...
        return hasSection(Q.section().select(Section.PAGE_PROPERTY).filterById(QueryOperator.IN, (Object[])section));
    }
    ...
    public PageRequest hasSection(SectionRequest section) {
        return hasSection(section, $section -> java.util.Optional.of($section).map(Section::getPage).map(Page::getId).orElse(null));
    }
    ...
    public PageRequest hasSection(
        SectionRequest section, QueryOperator queryOperator, Object value) {
      return addSearchCriteria(
          new AggregationCriteria(
              section, Section.PAGE_PROPERTY, queryOperator, value));
    }
