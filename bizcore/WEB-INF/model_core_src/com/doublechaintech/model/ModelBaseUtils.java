...
import static com.doublechaintech.model.controller.ViewRender.UI_CANDIDATE_ATTRIBUTE_PREFIX;
...
public class ModelBaseUtils {
  ...
  public static <T> T orElse(Callable<T> call, T defaultValue) {
	...
	public static String getCacheAccessKey(ModelUserContext ctx) {
	...
	protected static BaseEntity loadCanCacheInLocal(ModelUserContext userContext, String type, String id) throws Exception {
	...
	public static <T extends BaseEntity> List<T> collectReferencedObjectWithType(ModelUserContext userContext,
	...
	private static <T extends BaseEntity> void collectReferedObjectIdSet(ModelUserContext userContext,
	...
	public static int getAppBuildVersion(ModelUserContext ctx) {
		return getBuildVersion(getRequestAppVersion(ctx));
	}
	protected static boolean startFromVersion(ModelUserContext ctx, int version) {
     ...
     * "x-app-version" : "9"
	 */
	public static String getRequestAppVersion(ModelUserContext userContext) {
		return userContext.getRequestHeader("x-app-version");
	}
	public static String getRequestAppDevice(ModelUserContext userContext) {
		return userContext.getRequestHeader("x-app-device");
	}
	public static String getRequestAppType(ModelUserContext userContext) {
	...
	public static PageType getPageType(ModelUserContext userContext, String code) throws Exception {
	...
	public static <T> T loadBaseEntityById(ModelUserContext ctx, String type, String id) throws Exception {
		return loadBaseEntityById(ctx, type, id, null);
	}
	public static <T> T loadBaseEntityById(ModelUserContext ctx, String type, String id, Consumer<T> enhancer) throws Exception {
		String key = type+":"+id;
	...
	public static <T extends BaseEntity> void appendLinkToUrl(ModelUserContext ctx, List<T> list,
			Function<T, String> makeFunc) {
		if (list == null || list.isEmpty()) {
			return;
		}
		list.forEach(it -> {
			it.addItemToValueMap(ModelBaseConstants.X_LINK_TO_URL, makeFunc.apply(it));
		});
	}

	public static <T extends BaseEntity> void appendLinkToUrl(ModelUserContext ctx, T obj, String url) {
		if (obj == null) {
			return;
		}
		obj.addItemToValueMap(ModelBaseConstants.X_LINK_TO_URL, url);
	}

	public static ButtonViewComponent addAction(ModelUserContext ctx, BaseEntity obj, String title, String code,
			String linkToUrl) {
	...
	public static void setAction(ModelUserContext ctx, BaseEntity obj, String title, String code, String linkToUrl) {
		ButtonViewComponent actionBtn = new ButtonViewComponent(title, null, code);
		actionBtn.setLinkToUrl(linkToUrl);
    ...
    public static void ensureTable(ModelUserContext userContext, Map<String, Object> result,
  		...
  		// throw new Exception("ModelBaseUtils::ensureTable()尚未实现");

        DBQuerier querier = (DBQuerier) userContext.getBean("dbQuerier");
        Map<String, Object> checkResult = DBChecker.verifyTable("model",tableName, title, fieldInfo, indexSqls, constraintSqls, querier);
    ...
    public static void ensureDataByCode(ModelUserContext userContext, Map<String, Object> result,
  					String tableName, String[][] rowData) throws Exception{
        // throw new Exception("ModelBaseUtils::ensureDataByCode()尚未实现");
        DBQuerier querier = (DBQuerier) userContext.getBean("dbQuerier");
 ...
 public static void enhanceRequest(BaseRequest request) {
  ...
  public static void createSubRequests(BaseRequest request) {
    ...
    for (Class directSubClass : directSubClasses) {
      ...
      BaseRequest child = (BaseRequest) request.getChildrenSelects().get(simpleRequest.getInternalType() + parentLinkName);
      if (child == null) {
        child = simpleRequest;
  ...
  public static <T extends BaseEntity> T buildItem(CustomModelUserContextImpl ctx, Class<T> type) {
    ...
    private static <T extends BaseEntity> T buildItemFromBody(
        CustomModelUserContextImpl ctx, Class<T> type) {
    ...
    private static <T extends BaseEntity> T buildItemFromFormData(
        CustomModelUserContextImpl ctx, Class<T> type, String requestParameter) {
      Gson gson = new Gson();
    ...
    private static String getFormData(CustomModelUserContextImpl ctx) {
  ...
  public static <T extends BaseEntity> T buildItem(
      CustomModelUserContextImpl ctx, Map<String, Object> data, Class<T> type) {
    ...
    if (!ObjectUtil.isEmpty(req)) {
      ...
      if (cachedObject == null) {
        ...
        throw new com.doublechaintech.model.form.DuplicatedFormException("你的请求已处理，请勿重复提交！");
      }
    }
    try {
  ...
  public static <T extends BaseEntity> T ensure(ModelUserContext ctx, T item, Consumer<T> enhancer)
      throws Exception {
    if (item == null) {
      return null;
    }
  ...
  public static <T extends BaseEntity> T saveItem(ModelUserContext ctx, T pItem) throws Exception {
    if (pItem == null) {
      return null;
    }
    ReflectUtil.invoke(ctx.getChecker(), "checkAndFix" + pItem.getInternalType(), pItem);
    ctx.getChecker().throwExceptionIfHasErrors(ModelException.class);
  ...
  // 只处理string或引用类型的相等
  public static <T extends BaseEntity> T reload(ModelUserContext ctx, T pItem) throws Exception {
    BaseRequest request = createReloadRequest(pItem);
    return (T) Searcher.searchOne(ctx, request);
  }

  public static <T extends BaseEntity> T reloadSelf(ModelUserContext ctx, T pItem) throws Exception {
  ...
  public static <T extends BaseEntity> void saveItemSelf(ModelUserContext ctx, T... pItems)
      throws Exception {
    saveItemSelf(ctx, ListUtil.of(pItems));
  }

  public static <T extends BaseEntity> void saveItemSelf(ModelUserContext ctx, List<T> pItems)
      throws Exception {
  ...
  public static <T extends BaseEntity> void saveGraph(ModelUserContext ctx, T... pItems)
      throws Exception {
    saveGraph(ctx, ListUtil.of(pItems), new ArrayList<>());
  }

  public static void saveGraph(ModelUserContext ctx, List<BaseEntity> pItems) throws Exception {
    saveGraph(ctx, pItems, new ArrayList<>());
  }

  private static void saveGraph(
      ModelUserContext ctx, List<BaseEntity> nodes, List<BaseEntity> handled) throws Exception {
