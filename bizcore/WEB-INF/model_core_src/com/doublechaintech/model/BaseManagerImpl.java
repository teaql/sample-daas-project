...
public class BaseManagerImpl implements AccessControledService,BeanNameAware{
	...
	public Optional<UserApp> findOutUserApp(ModelUserContext userContext, String userAppId) throws Exception {
	...
	public Object accessUserApp(BaseUserContext userContext,String targetUserAppId) throws Exception{
		ModelUserContext dmosUserContext=(ModelUserContext)userContext;
	...
	public void ensureRelationInGraph(UserContext userContext, BaseEntity sourceEntity) {
		...
		GraphService graphService = ((ModelUserContextImpl)userContext).getGraphService();
  ...
  //		((GraphServiceImpl) graphService).addEdge(getSystemInternalName(), sourceEntity.getInternalType() + "_" + entity.getInternalType(), sourceEntity.getInternalType(), entity.getInternalType(), sourceEntity.getId(), entity.getId());
	...
	public void sendCreationEvent(UserContext userContext, BaseEntity sourceEntity) {
		EventService eventService = ((ModelUserContextImpl)userContext).getEventService();
	...
	protected Object invokeWithOneString(BaseUserContext baseUserContext,
		...
		Method  method =target.getClass().getMethod(methodName, (Class<?>[]) new Class<?>[]{ModelUserContext.class,String.class});
...
您输入的出生日期格式有误，该输入正确的的格式是像 2017-09-09的日期
    ...
    protected void checkBooleanRange(boolean pFullAccess, boolean pI, boolean pB, String pFullAccessOfUserApp,
            List<Message> messageList) {
	...
	public void onAccess(BaseUserContext baseUserContext, String methodName, Object[] parameters) {
		ModelUserContextImpl ctx = (ModelUserContextImpl) baseUserContext;
	...
	public void afterInvoke(BaseUserContext baseUserContext, String methodName, Object[] parameters, boolean success, Object result, Throwable throwable) {
  	ModelBizUserContextImpl ctx = (ModelBizUserContextImpl) baseUserContext;
    if (throwable != null) {
  ...
  protected <T> T orNull(Callable<T> call) {
		return ModelBaseUtils.orNull(call);
	}

	protected <T> T ifNull(T call, T defaultValue) {
		return ModelBaseUtils.ifNull(call, defaultValue);
	}

	protected <T> T orElse(Callable<T> call, T defaultValue) {
		return ModelBaseUtils.orElse(call, defaultValue);
	}


	// -----------------------------------//  登录部分处理 \\-----------------------------------
	// 调试时直接使用ID登录, 注意是 fullId
	public Object loginById4Debug(ModelUserContext userContext, String fullId)
            throws Exception {
      LoginChannel loginChannel =
              LoginChannel.of(
                      ModelBaseUtils.getRequestAppType(userContext), this.getBeanName(), "loginById");
	...
	// 手机号+短信验证码 登录
	public Object loginByMobile(ModelUserContext userContext, String mobile, String verifyCode) throws Exception {
		LoginChannel loginChannel = LoginChannel.of(ModelBaseUtils.getRequestAppType(userContext), this.getBeanName(),
				"loginByMobile");
		LoginData loginData = new LoginData();
	...
	// 账号+密码登录
	public Object loginByPassword(ModelUserContext userContext, String loginId, Password password) throws Exception {
		LoginChannel loginChannel = LoginChannel.of(ModelBaseUtils.getRequestAppType(userContext), this.getBeanName(), "loginByPassword");
		LoginData loginData = new LoginData();
	...
	// 微信小程序登录
	public Object loginByWechatMiniProgram(ModelUserContext userContext, String code) throws Exception {
		LoginChannel loginChannel = LoginChannel.of(ModelBaseUtils.getRequestAppType(userContext), this.getBeanName(),
	...
	// 微信小程序获取手机号快速登录
  public Object loginByWechatMobile(ModelUserContext userContext, String code, String encryptedData, String iv)
          throws Exception {
    LoginChannel loginChannel =
            LoginChannel.of(
                    ModelBaseUtils.getRequestAppType(userContext),
                    this.getBeanName(),
                    "loginByWechatMobile");
    LoginData loginData = new LoginData();
    loginData.setCode(code);
	...
	// 企业微信小程序登录
	public Object loginByWechatWorkMiniProgram(ModelUserContext userContext, String code) throws Exception {
		LoginChannel loginChannel = LoginChannel.of(ModelBaseUtils.getRequestAppType(userContext), this.getBeanName(),
	...
	// 调用登录处理
	protected Object processLoginRequest(ModelUserContext baseUserContext, LoginContext loginContext) throws Exception {
		ModelUserContextImpl userContext = (ModelUserContextImpl)baseUserContext;
	...
	protected boolean isMethodNeedLogin(ModelUserContext userContext, String methodName, Object[] parameters) {
	...
	protected void afterSecUserAppLoadedWhenCheckAccess(ModelUserContext userContext, Map<String, Object> loginInfo,
			SecUser secUser, UserApp userApp) throws IllegalAccessException{
	}

	protected Object onLoginSuccess(ModelUserContext userContext, LoginResult loginResult) throws Exception {
		// by default, return the view of this object
		UserApp userApp = loginResult.getLoginContext().getLoginTarget().getUserApp();
		return this.view(userContext, userApp.getAppId());
	}

  public void onAuthenticateUserLogged(ModelUserContext userContext, LoginContext loginContext,
    ...
    ((ModelBizUserContextImpl)userContext).setCurrentUserInfo(app);
  }

  public Object view(ModelUserContext userContext, String id) throws Exception {
