...
public abstract class BaseIamService {
	...
	public LoginResult doLogin(ModelUserContextImpl userContext, LoginContext loginContext, BusinessHandler bizHandler) throws Exception {
  ...
  public Object doLogout(ModelUserContextImpl userContext, BusinessHandler bizHandler) throws Exception {
		// 回调业务
	 ...
	 * @throws Exception
	 */
	protected void onAuthenticateNewUserLogged(ModelUserContextImpl userContext, LoginContext loginContext,
	 ...
	 *
	 */
	protected void onAuthenticateUserLogged(ModelUserContextImpl userContext, LoginContext loginContext,
			LoginResult loginResult, IdentificationHandler idHandler, BusinessHandler bizHandler) throws Exception {
		// 回调业务
	...
	// 为兼容旧版本secUser登录接口
	public void onLoginSecUserInternalSuccess(ModelUserContextImpl userContext, SecUser user) {
		String tokenKey = userContext.tokenId();
		// 生成 jwtToken
	...
	protected String makeUserUploadHomeWhenLogged(SecUser secUser, UserApp userApp) {
		if (userApp == null && secUser == null) {
			return "upload/model/anonymous";
		}
		if (secUser != null) {
			return String.format("upload/model/%s/%s", SecUser.INTERNAL_TYPE, secUser.getId());
		}
		return String.format("upload/model/%s/%s", userApp.getAppType(), userApp.getAppId());
	}
	 ...
	 */
	protected boolean isLoginClientSupportSession(ModelUserContextImpl userContext, LoginContext loginContext,
	 ...
	 */
	protected void onAuthenticationFailed(ModelUserContextImpl userContext, LoginContext loginContext, LoginResult loginResult,
	 ...
	 * @return
	 */
	protected IdentificationHandler selectIdentificationHandler(ModelUserContextImpl userContext, LoginContext loginContext) {
	  if (loginContext.getLoginMethod().equals(LoginMethod.ID_4_DEBUG)){
        return new IdentificationHandler() {
          ...
          public LoginResult authenticate(
              ModelUserContext userContext, LoginContext loginContext) {
            ...
            UserApp userApp = ModelUserContextImpl.getUserAppByBindedEntity(userContext, BaseEntity.pretendToBe(objType, objId));
            if (userApp == null){
          ...
          @Override
          public void bindWithSecUser(ModelUserContext userContext, LoginContext loginContext)
              throws Exception {
            // nothing to do
          }
        };
	...
	protected String getLoginInfoCacheKey(ModelUserContextImpl userContext) {
		return this.getCacheKey(userContext, "loginInfo");
	}
	protected String getJwtTokenCacheKey(ModelUserContextImpl userContext) {
		return this.getCacheKey(userContext, "jwtToken");
	}

	// 获取token key的优先级为: jwtToken->cookie->session id
	public String getTokenKey(ModelUserContextImpl userContext) {
	...
	protected void cacheJwtToken(ModelUserContextImpl userContext, DecodedJWT jwtToken) {
		userContext.putIntoContextLocalStorage(getJwtTokenCacheKey(userContext), jwtToken);
	}
	protected DecodedJWT getCachedJwtToken(ModelUserContextImpl userContext) {
		return (DecodedJWT) userContext.getFromContextLocalStorage(getJwtTokenCacheKey(userContext));
	}

	protected DecodedJWT getJwtTokenFromCookie(ModelUserContextImpl userContext) {
	...
	protected DecodedJWT getJwtTokenFromHeader(ModelUserContextImpl userContext) {
	...
	public Map<String, Object> getCachedLoginInfo(ModelUserContextImpl userContext) {
		getTokenKey(userContext);
		String cacheKey = getLoginInfoCacheKey(userContext);
	...
	protected String getCacheKey(UserContext userContext, String prefix){
		return "model:"+prefix+":"+userContext.tokenId();
	}
	protected String getUserKey(UserContext userContext){
		return "model:user:"+userContext.tokenId();
	}
	protected String getCurrentAppKey(UserContext userContext){
		return getUserKey(userContext)+":currentApp";
	}
	protected void fixUserContextTokenId(ModelUserContextImpl userContext, String tokenKey) {
		if (!userContext.tokenId().equals(tokenKey)) {
			userContext.setTokenId(tokenKey);
		}
	}
	public SecUser tryToLoadSecUser(ModelUserContextImpl userContext, Map<String, Object> loginInfo){
		if(loginInfo == null) {
	...
	public UserApp tryToLoadUserApp(ModelUserContextImpl userContext, Map<String, Object> loginInfo){
		if(loginInfo == null) {
	 ...
	 * @param loginInfo */
	public void onCheckAccessWhenAnonymousFound(ModelUserContextImpl userContext, Map<String, Object> loginInfo) {
	...
	public Object forceLoginAs(ModelUserContextImpl userContext, BaseEntity loginTarget, BusinessHandler bizHandler) throws Exception {
      if (userContext.isProductEnvironment()) {
        throw new Exception("只有测试环境才可使用此方法");
      }
      UserApp userApp = ModelUserContextImpl.getUserAppByBindedEntity(userContext, loginTarget);
      if (userApp == null){
    ...
    public void selectUserApp(ModelUserContext userContext, UserApp userApp) throws Exception {
      ModelUserContextImpl ctx = (ModelUserContextImpl)userContext;
