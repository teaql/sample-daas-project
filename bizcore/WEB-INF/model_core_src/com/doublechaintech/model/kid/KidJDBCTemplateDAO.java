...
public class KidJDBCTemplateDAO extends ModelBaseDAOImpl implements KidDAO {
  ...
  */
  ...
  protected Kid loadInternalKid(AccessKey accessKey, Map<String, Object> loadOptions)
      throws Exception {
  ...
  public SmartList<Kid> findKidByFamily(String familyId, Map<String, Object> options) {
    ...
    SmartList<Kid> resultList =
        queryWith(KidTable.COLUMN_FAMILY, familyId, options, getKidMapper());
  ...
  public SmartList<Kid> findKidByFamily(
    ...
    SmartList<Kid> resultList =
        queryWithRange(KidTable.COLUMN_FAMILY, familyId, options, getKidMapper(), start, count);
    // analyzeKidByFamily(resultList, familyId, options);
    return resultList;
  }
  ...
  protected Object[] splitKidList(List<Kid> kidList) {
    ...
    return new Object[] {kidCreateList, kidUpdateList, kidRemoveList, kidRecoverList};
  }
  ...
  protected Object[] prepareKidUpdateParameters(Kid kid) {
    ...
    parameters[1] = kid.getAge();

    if (kid.getFamily() != null) {
      parameters[2] = kid.getFamily().getId();
    }

    parameters[3] = kid.nextVersion();
    parameters[4] = kid.getId();
    parameters[5] = kid.getVersion();
  ...
  protected Object[] prepareKidCreateParameters(Kid kid) {
    ...
    parameters[2] = kid.getAge();

    if (kid.getFamily() != null) {
      parameters[3] = kid.getFamily().getId();
    }
