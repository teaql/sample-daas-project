...
import static com.doublechaintech.model.loginhistory.LoginHistory.*;

public class LoginHistoryRequest extends BaseRequest<LoginHistory> {
    public static LoginHistoryRequest newInstance() {
        return new LoginHistoryRequest().filterByVersion(QueryOperator.GREATER_THAN,0).selectId();
    }
    ...
    public LoginHistoryRequest addDynamicProperty(String propertyName, BaseRequest request, String parentPropertyName) {
    ...
    public LoginHistoryRequest selectAny() {return selectAll();
        }
    ...
    protected static final String[] ALL_SELF_FIELDS={ID_PROPERTY,LOGIN_TIME_PROPERTY,FROM_IP_PROPERTY,DESCRIPTION_PROPERTY,SEC_USER_PROPERTY,VERSION_PROPERTY};
    ...
    public LoginHistoryRequest whereLoginTimeIsNotNull() {
      ...
      public LoginHistoryRequest whereLoginTimeBetween(Date loginTimeStart, Date loginTimeEnd) {
          if (ObjectUtil.isEmpty(loginTimeStart)) {
               throw new IllegalArgumentException("Method whereLoginTimeBetween, the parameter loginTimeStart is not allowed  to be null.");
          }
          if (ObjectUtil.isEmpty(loginTimeEnd)) {
               throw new IllegalArgumentException("Method whereLoginTimeBetween, the parameter loginTimeEnd is not allowed  to be null.");
          }
          SearchCriteria searchCriteria = getLoginTimeSearchCriteria(QueryOperator.BETWEEN,  new Object[]{loginTimeStart, loginTimeEnd});
          return addSearchCriteria(searchCriteria);
      }
      ...
      public LoginHistoryRequest whereLoginTimeBefore(Date loginTime) {
          ...
          if (ObjectUtil.isEmpty(loginTime)) {
               throw new IllegalArgumentException("Method whereLoginTimeBefore, the parameter loginTime is not allowed  to be null.");
          }

          SearchCriteria searchCriteria = getLoginTimeSearchCriteria(QueryOperator.LESS_THAN, new Object[]{loginTime});
          return addSearchCriteria(searchCriteria);
      }
      ...
      public LoginHistoryRequest whereLoginTimeAfter(DateTime loginTime) {
          if (ObjectUtil.isEmpty(loginTime)) {
              throw new IllegalArgumentException("Method whereLoginTimeAfter, the parameter loginTime is not allowed  to be null.");
          }
          SearchCriteria searchCriteria = getLoginTimeSearchCriteria(QueryOperator.GREATER_THAN, new Object[]{loginTime});
          return addSearchCriteria(searchCriteria);
      }
